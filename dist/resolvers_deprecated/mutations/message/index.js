...
async function createMessage(parent, args, context, info) {
  ...
  const conversationUsers = await context.prisma.conversation({
    id: conversationId
  }).participants();
  ...
  const results = conversationUsers.filter(user => user.id !== userId).map(async user => {
    MESSAGE_SENT.title = `Message from ${senderDetails.name}`;
    const notificationExists = await createNotification(MESSAGE_SENT, user.id, context);
    !notificationExists && (await emailNewMessage(user, MESSAGE_SENT.message));
  });
